...
void man_flag_id(va_list list, const char *format, int *i)
{
    for (; format[*i] && check_flag_id(format[*i]); *i += 1) {
        if (format[*i] == '#')
            ash_f(format[*i + 1]);
        if (format[*i - 1] == '#')
    ...
    for (int i = 0; format[i]; i++) {
        if (format[i] == '%' && check_flag_id(format[i + 1]))
            if (format[i + 1] == 'n')
                ...
                man_flag_id(list, format, &i);
            else
                call_flag(list, format[i++ + 1]);
        else if (format[i] == '%' && !check_flag_id(format[i + 1]))
            my_putchar(format[i++]);
        else
            my_putchar(format[i]);
    }
